...
abstract class CalendarPagerAdapter<V extends CalendarPagerView> extends PagerAdapter {
	...
	private List<CalendarDay> flagDates = new ArrayList<CalendarDay>();
	...
	//设置flagDate
	private List<CalendarDay> flagDateList;
	public void setFlagDateList(List<CalendarDay> dates){
		this.flagDateList = dates;
	}
	...
	public void setFlagDates(List<CalendarDay> dates){
		flagDates.clear();
		if (dates != null) {
			flagDates.addAll(dates);
		}
		for (V pagerView : currentViews) {
			pagerView.setFlagDates(selectedDates);
		}
	}
