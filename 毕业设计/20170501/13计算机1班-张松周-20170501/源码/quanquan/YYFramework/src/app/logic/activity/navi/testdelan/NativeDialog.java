...
public class NativeDialog extends Dialog {
    ...
    public NativeDialog(Context context, Location loc_now, Location loc_end) { //View.OnClickListener context
	...
	private void setMsgDialog() {
    	...
    	if (apps!=null && apps.size()!=0) {
            ...
            if(title!=null) title.setText(msg);  //头 标题  “选择您需要打开的应用”
            if(negativeButton!=null) negativeButton.setText(negativeStr);    //取消按钮
            if(negativeButton!=null) negativeButton.setOnClickListener(deflistener);  //取消按钮监听
    		...
    		for (int i = 0; i < apps.size(); i++) {
                ...
                drawable.setBounds(0, 0, 100, 100 );                            // 这一步必须要做,否则不会显示
                textView.setCompoundDrawables( null , drawable , null , null ); // （上边）
    			//textView.setCompoundDrawablesWithIntrinsicBounds(null,app.getAppIcon(), null, null);	//设置图标（上边）
    			...
    			textView.setOnClickListener(applistener);						//设置监听  点击跳转
                textView.setTextSize(14.0f);  //在样式R.style.text_small_dark中有设置为16sp,为何会变大？
    			views.add(textView);
            ...
            //if(positiveButton!=null) positiveButton.setOnClickListener(weblistener);  //继续导航
            if(negativeButton!=null) negativeButton.setOnClickListener(deflistener);  //取消导航
		}
        super.setContentView( mView );
    }
