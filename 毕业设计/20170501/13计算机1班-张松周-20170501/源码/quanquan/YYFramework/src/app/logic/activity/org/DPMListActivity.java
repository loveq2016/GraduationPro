...
public class DPMListActivity extends ActActivity implements IXListViewListener, OnItemClickListener, OnClickListener, OnTouchListener {
    ...
    private List<DepartmentInfo> setDPMList;    //组织下的部所有部门列表
...
//                    Picasso.with(DPMListActivity.this).load(HttpConfig.getUrl(info.getOrgRequestMemberInfo().getPicture_url())).error(R.drawable.default_user_icon).fit().centerCrop().into(headIv);
    ...
    private void updataUI() {
        ...
        dataList.clear();
        getDPM(org_id);          //获取组织中的部门列表
    }

    /**
     * @param org_id 组织ID
     *               getDPMDPMListActivity 获取部门列表
     */
...
//                }
    ...
    private void fillDatas(List<OrgRequestMemberInfo> list) {
        ...
        List<DepartmentInfo> noDpm = new ArrayList<DepartmentInfo>();    // 没有部门的成员的集合
        ...
        if (noDpm.size() > 0) {
            ...
            dataList.addAll(noDpm); //添加不属于任何部门的 成员
        }
    ...
    private void showPopupWindow(View v) {
        if (popupWindow_more == null) {
            ...
            if (!isBulider) {  //当前用户不是个组织的创建者时，就隐藏下面三个字段
                ...
                orgAddDpm_ll.setVisibility(View.GONE);    //添加部门
            }
...
//                    expansionInfo.setItemName(nameString);
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
       ...
       if (info.isBuilder() || info.isNotDpmAndNotAdmin() ||info.isAdmin()) {// 点击的是超级管理员、管理员、不属于任何部门的成员 de 也不能点
            ...
            intent.putExtra(DPMDetailsActivity.KDPM_ID, info.getDpm_id());  //部门ID
            intent.putExtra(DPMDetailsActivity.KTITLE, info.getDpm_name()); //部门名称
    ...
    private void showSetDPMList(Context context, List<DepartmentInfo> list, String user_id, final int position) {
        if (list == null || list.size() < 1) {
            QLToastUtils.showToast(context, "没有部门可以加进");
            return;
        }
    ...
    /**
     * 超级管理员移除不属于任何部门的成员时 显示移除对话框提示信息
     *
     * @param text
     * @param position
     * @param info
        ...
        yesBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (info != null) {
                    ...
                    removeDPM(position);  //移除没有添加到任何部门的成员的方法（侧滑菜单 移除 ）
                }
                alertDialog.dismiss();
            }
        });
    ...
    /**
     * 移除没有添加到任何部门的成员的方法（侧滑菜单 移除 超级管理员才可以）
     *
     * @param org_id
     * @param member_id
     */
