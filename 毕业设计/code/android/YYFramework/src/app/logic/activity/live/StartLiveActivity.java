...
//    ViewGroup finish_frame ;
    ...
    @Override protected void onActivityCreate(@Nullable Bundle savedInstanceState) {
        ...
        //初始化配置（开启直播的流ID）
        liveStreamId = System.currentTimeMillis() + "" ;
        //初始化配置
        initConfig() ;
        //初始对话框（关闭直播对话框）
        intiDialog();
    ...
    private void initConfig() {
        mRtmpAddress = rtmpPushStreamDomain  + liveStreamId ;
    ...
    void sheraLive() {
        ShareHelper.showShare(this,"我的【格局】看到一个不错的直播，向您推荐一下！",orgName+"直播",orglog,HttpConfig.SHARE_LIVE+liveId);
//        showShare( orgName+"直播" );
    }
    ...
    private void createLiveStream(String org_id , String room_id  ) {
        LivestreamController.createLiveStream(this, org_id , room_id ,liveStreamId ,liveId, new Listener<Boolean, String>() {
            @Override
            ...
            public void onCallBack(Void status, List<OrgRequestMemberInfo> reply) {
                if (reply != null && reply.size() > 0) {
                    for( int i = 0 ; i <reply.size() ; i++){
                        if( !TextUtils.isEmpty( reply.get( i).getWp_member_info_id() )){
                            orgMemberList.put( reply.get( i).getWp_member_info_id() ,reply.get( i));
                            messageView.orgMemberList.put( reply.get(i).getWp_member_info_id(),reply.get( i ));
                        }
                    }
                }
            }
