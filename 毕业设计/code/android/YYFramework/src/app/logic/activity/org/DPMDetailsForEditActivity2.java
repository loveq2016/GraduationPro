...
/*
 * GZYY    2016-12-22  下午4:31:49
 * author: zsz
 */

public class DPMDetailsForEditActivity2 extends ActActivity implements View.OnClickListener {
    ...
    private UserInfo adniminfo; //管理员
    ...
    private View dpmllView, contentView, dpm_admin_and_img, adminView, dpmView; //分组名称 ， 对分组话框名称 ， 分组管理员 ， 设置管理员时对话框View
    private Button trueBtn, cancelBtn, hAdminBtn, nAdminBtn, noBtn, delBtn;
    private EditText contentEdt;
    private DialogNewStyleController dialog, adminDialog, dpmDialog;

    private TextView dpmNameEdt;        //分组的名字
    private TextView adminNameTv;       //分组管理员的名字
    private ImageView addAdminIv;       //添加分组管理员的图片按钮
    private GridView memberGv;          //显示成员头像
    private Button dpmBtn;              //解散分组 、确认按钮
    private boolean editStatus = true; //是否可编辑的标志位
    ...
    private boolean isAdmie = false;    //分组管理员 ？

    private String wp_member_info_id;    //被设置为管理员的Id
    private String dpm_id, dpm_name, adinmeName;    //分组Id ,名称
    ...
    private void initDatas() {
        ...
        adniminfo = (UserInfo) getIntent().getSerializableExtra(ANDIMINFO);
        ...
        if (adniminfo != null) {
            wp_member_info_id = adniminfo.getWp_member_info_id();
        }
    }
    ...
    private void initView() {
        ...
        intiDiglog(contentView);     //分组名称对话框
        adminView = LayoutInflater.from(this).inflate(R.layout.dialog_new_admincontent_view, null);
        intiAdminDialog(adminView);  //分组管理员对话框
        dpmView = LayoutInflater.from(this).inflate(R.layout.dialog_new_dpmcontent_view, null);
        intiDpmDialog(dpmView);
        ...
        dpmNameEdt.setEnabled(true);
        dpmNameEdt.setOnClickListener(this);
        ...
        if (intentInfo.getOpenMode() == IntentInfo.ADD_MODE) {
            ...
            editStatus = true;
            changeEditOrCancel();
        } else if (intentInfo.getOpenMode() == IntentInfo.EDIT_MODE) {

            if (adinmeName != null) {
                adminNameTv.setText(adinmeName);
            } else {
                adminNameTv.setText("");
            }
            if (isBuilder) {
                ...
                addAdminIv.setVisibility(View.VISIBLE);
            } else {
                dpmBtn.setVisibility(View.GONE);
                addAdminIv.setVisibility(View.GONE); //只有超级管理员才可以设置管理员
            }
            ...
            dpmNameEdt.setText(dpm_name); //分组名称
        }
        //设置适配器
    ...
    private void intiDiglog(View contentView) {
        ...
        if (intentInfo.getOpenMode() == IntentInfo.ADD_MODE) {
            ...
            contentEdt.setHint("填写要创建的分组昵称");
        } else {
            title.setText("修改分组名称");
            contentEdt.setText(dpm_name);
        }
        ...
        trueBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(newDpaName)) {
                    ...
                    QLToastUtils.showToast(DPMDetailsForEditActivity2.this, "分组名称不能为空");
                }
            }
        });
     ...
     * @param
     */
    private void intiDpmDialog(View contentView) {
        TextView title = (TextView) contentView.findViewById(R.id.dialog_title_tv);
        delBtn = (Button) contentView.findViewById(R.id.dialog_true_btn);
        noBtn = (Button) contentView.findViewById(R.id.dialog_cancel_btn);
        dpmDialog = new DialogNewStyleController(this, contentView);
        title.setText("确定要解散该分组吗？");
        //取消按钮
    ...
    private void addListener() {
        //添加管理員按鈕
        addAdminIv.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if (editStatus) {
                    if (!TextUtils.isEmpty(adminNameTv.getText().toString())) { //当前分组有管理员时，在点击的时候就显示对话框
                        adminDialog.show();
                    } else {
                        ...
                        Intent intent = new Intent(DPMDetailsForEditActivity2.this, LogicFriends.class);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (intentInfo.getOpenMode() == IntentInfo.ADD_MODE) {    //新增模式下（添加部門接賣弄跳轉過來的）
                    ...
                    if (position == datas.size() - 1) {             // 刪除  點擊的是刪除圖片
                        ...
                        intent.putExtra(LogicFriends.DATAS_LIST, dataString);
                        intent.putExtra(LogicFriends.TITLE, "分组成员");
        ...
        //按鈕監聽器
        dpmBtn.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                    ...
                    if (intentInfo.getOpenMode() == IntentInfo.EDIT_MODE) {  //修改模式
                        if (editStatus) {
                            if (isBuilder) {  //協會的創建愛這才可以解散部門
                                delDpm();
                                //removeDpm( intentInfo.getOrgId() , intentInfo.getDpmId()); //解散分组
                            } else {
                                finish();
                            }
                        } else {
    ...
    protected void onResume() {
        ...
        super.onResume();
        if (intentInfo.getOpenMode() == IntentInfo.EDIT_MODE && isResult ) {
            isResult = false ;
            getDPMMember();  //获取本分组成员
        }
    }
...
//				if( isAdmie && adniminfo != null ){  //移除当前管理员
//					delAdmie( datas ); //移除指定对象
//				}
                if (editStatus) {
                    addLastItem();
                }
     ...
     * @param list
     */
    private void delAdmie(List<ExpansionInfo> list) {
        if (list == null || adniminfo == null) {
            return;
        }
        for (ExpansionInfo info : list) {
            if (info.getWp_member_info_id().equals(adniminfo.getWp_member_info_id())) {
                datas.remove(info);
                return;
            }
        }
     ...
     * @return 获取当前分组的成员, 返回json getSelectMemberListDPMDetailsForEditActivity2
     */
...
//                expansionInfo.setItemID(expansionInfo.getUserInfo().getWp_member_info_id());
            ...
            if ( type == 0 ) {
                expansionInfo.setItemShowCheck(false);  //设置分组管理员时，复选框不显示
            } else {
                expansionInfo.setItemShowCheck(true);   //選擇的不是管理員，（删除或添加成员都要显示复选框）
            }
            //过滤掉用户自己
            if(UserManagerController.getCurrUserInfo().getWp_member_info_id().equals( expansionInfo.getWp_member_info_id() )){
                continue ;
            }
            expansionInfo.setItemIsCheck(false);  //itme是否处于选择状态
            memberList.add(expansionInfo);
        }

//        if (type == 1 && wp_member_info_id != null && !TextUtils.isEmpty(wp_member_info_id) && isAdmie) {
//            Iterator iterator = memberList.iterator();
//            while (iterator.hasNext()) {
//                ExpansionInfo expansionInfo = (ExpansionInfo) iterator.next();
//                if (expansionInfo.getWp_member_info_id().equals(wp_member_info_id)) {
//                    iterator.remove();  //移除当前分组的管理员
//                }
//            }
//        }
...
//				removeLastItem();   //移除最后两张图片
    ...
    private void addMemberToDpm(final Intent intent) {
        OrganizationController.getOrgMemberList(this, intentInfo.getOrgId(), new Listener<Void, List<OrgRequestMemberInfo>>() {
            ...
            public void onCallBack(Void status, List<OrgRequestMemberInfo> reply) {
                ...
                if (lastInfos.size() < 1) {
                    QLToastUtils.showToast(DPMDetailsForEditActivity2.this, "没有可以添加的成员");
                    return;
                }
                ...
                for (OrgRequestMemberInfo memberInfo : lastInfos) {
                    ...
                    if (temp) {
                        ...
                        expansionInfo.setItemName(memberInfo.getName());
                        expansionInfo.setNickName(memberInfo.getNickName());
                        expansionInfo.setFriend_name(memberInfo.getFriend_name());
     ...
     *               addMemberToDpmDPMDetailsForEditActivity2 添加成员到分组
     */
    private void addMemberToDpm(String result, int type) {
        if (TextUtils.isEmpty(result)) {
            return;
        }
        final List<ExpansionInfo> newInfos = gson.fromJson(result, new TypeToken<List<ExpansionInfo>>() {}.getType());
            ...
            OrganizationController.addMemberToDPM(this, intentInfo.getOrgId(), intentInfo.getDpmId(), builder.toString(), new Listener<Integer, String>() {
                ...
                public void onCallBack(Integer status, String reply) {
                    if (status != 1) {
                        ...
                        QLToastUtils.showToast(DPMDetailsForEditActivity2.this, msg);
                    } else {
						datas.addAll( 0 , newInfos ); //添加在前面
						mAdapter.notifyDataSetChanged();
                        countGridViewHeight();
                        //getDPMMember();
                    }
        ...
        if (type == 1) {   //删
            filterList(newInfos);  //过滤集合（如果删除的成员中有管理员）is ?
            OrganizationController.removeMemberFromDPM(this, intentInfo.getOrgId(), intentInfo.getDpmId(), builder.toString(), new Listener<Integer, String>() {
                ...
                public void onCallBack(Integer status, String reply) {
                    if (status != 1) {
                        QLToastUtils.showToast(DPMDetailsForEditActivity2.this, reply == null ? "删除失败" : reply);
                    } else {
                        if (is && adniminfo != null) { //手动设置管理员不存在
                            adminNameTv.setText("");
                            addAdminIv.setImageDrawable(nDrawable);
                            adniminfo = null;
                            wp_member_info_id = null;
                        }
        ...
        if (type == 2) {   //添加管理员
            if (newInfos.size() > 0) {
                String name ="";
                if(newInfos.get(0).getFriend_name()!=null&&!TextUtils.isEmpty(newInfos.get(0).getFriend_name())){
                    name = newInfos.get(0).getFriend_name();
                }else{
                    name = newInfos.get(0).getNickName();
                }
    ...
    private void filterList(List<ExpansionInfo> list) {
        ...
        while (iterator.hasNext()) {
            ExpansionInfo expansionInfo = (ExpansionInfo) iterator.next();
            if (expansionInfo.getWp_member_info_id().equals(adniminfo.getWp_member_info_id())) {
                is = true;
                return;
            }
        }
    ...
    private void addMembetToNullDpm(String result, int type) {
        ...
        List<ExpansionInfo> newInfos = gson.fromJson(result, new TypeToken<List<ExpansionInfo>>() {}.getType());
        if (newInfos == null || newInfos.size() < 1) {
            return;
        }
        if (type == 0) {
        ...
        OrganizationController.addDPM(this, intentInfo.getOrgId(), dpmNameString, new Listener<String, DepartmentInfo>() {
            ...
            public void onCallBack(String status, DepartmentInfo reply) {
                if (TextUtils.isEmpty(status) || status.length() < 7) {
                    dismissWaitDialog();
                    QLToastUtils.showToast(DPMDetailsForEditActivity2.this, "分组创建失败，请重试");
                    return;
                }
                dpm_id = status; //分组创建成功后的分组ID
                ...
                if (iDBuilder.length() > 0) {
                    ...
                    //添加成员
                    OrganizationController.addMemberToDPM(DPMDetailsForEditActivity2.this, intentInfo.getOrgId(), status, iDBuilder.toString(), new Listener<Integer, String>() {
                        @Override
                        public void onCallBack(Integer status, String reply) {
                            if (status != 1) {
                                String msg = TextUtils.isEmpty(reply) ? "分组创建成功，添加成员失败" : reply;
                                QLToastUtils.showToast(DPMDetailsForEditActivity2.this, msg);
                                finish();
                            } else {
                                finish();
    ...
    private void updateDPM(String org_id, String departmentId, final String nameString) {
        showWaitDialog();  //添加分组
        OrganizationController.updateDepartment(DPMDetailsForEditActivity2.this, org_id, departmentId, nameString, new Listener<Boolean, String>() {
            @Override
            public void onCallBack(Boolean status, String reply) {
                ...
                if (status) {
                    ...
                    DPMDetailsActivity.mHandler.sendMessage(message);
                    dpmNameEdt.setText(nameString);
                } else {
                    dpmNameEdt.setText(dpm_name);
                    QLToastUtils.showToast(DPMDetailsForEditActivity2.this, reply == null ? "修改失败,请重试" : reply);
                }
            }
        });
    }
    ...
    private void setAdmieMthonADDMoed(String org_id, String dpm_id, String dpm_member_info_id, int type, final String name) {
        ...
        OrganizationController.setAdmin(DPMDetailsForEditActivity2.this, org_id, dpm_id, dpm_member_info_id, type, new Listener<Boolean, String>() {
            @Override
            public void onCallBack(Boolean aBoolean, String reply) {
                ...
                if (aBoolean) {
                    ...
                    QLToastUtils.showToast(DPMDetailsForEditActivity2.this, "设置成功");
                    adniminfo = new UserInfo();  //新的管理员
                    adniminfo.setWp_member_info_id(wp_member_info_id);
                } else {
                    adminNameTv.setText("");
                    QLToastUtils.showToast(DPMDetailsForEditActivity2.this, "设置失败");
                }
            }
        });
    }
        ...
        OrganizationController.setAdmin(DPMDetailsForEditActivity2.this, org_id, dpm_id, dpm_member_info_id, type, new Listener<Boolean, String>() {
            ...
            public void onCallBack(Boolean aBoolean, String reply) {
                if (aBoolean) {
                    adminNameTv.setText("");
                    addAdminIv.setImageDrawable(nDrawable);
                    adniminfo = null; //不存在管理员了
                    wp_member_info_id = null;
                } else {
                    QLToastUtils.showToast(DPMDetailsForEditActivity2.this, "修改失败");
                }
            }
        });
    }
    ...
    private void removeDpm(final String org_id, String dpm_id) {
        OrganizationController.removeDPM(this, org_id, dpm_id, new Listener<Integer, String>() {
            ...
            public void onCallBack(Integer status, String reply) {
                ...
                Intent intent = new Intent(DPMDetailsForEditActivity2.this, DPMListActivity.class);
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            if (getItemViewType(position) == 0) {
                ...
                if (convertView == null) {
                    ...
                    convertView = LayoutInflater.from(DPMDetailsForEditActivity2.this).inflate(R.layout.item_dpm_member, null);
...
//                    Picasso.with(DPMDetailsForEditActivity2.this).load(HttpConfig.getUrl(urlString)).error(R.drawable.default_user_icon).fit().centerCrop().into(holder.imageView);
//                    holder.imageView.setImageURI(HttpConfig.getUrl(urlString));
    ...
    private void addMenerToDmp(final List<ExpansionInfo> resrtList) {
        if (resrtList == null || resrtList.size() < 1) {
            QLToastUtils.showToast(DPMDetailsForEditActivity2.this, "请选择分组成员");
        }
        StringBuilder iDBuilder = new StringBuilder();
        ...
        if (iDBuilder.length() > 0) {
            iDBuilder.deleteCharAt(iDBuilder.length() - 1);
            //添加成员
            OrganizationController.addMemberToDPM(DPMDetailsForEditActivity2.this, intentInfo.getOrgId(), newDpmId, iDBuilder.toString(), new Listener<Integer, String>() {
                @Override
                public void onCallBack(Integer status, String reply) {
                    if (status != 1) {
