...
public class AddDPMActivity extends InitActActivity implements OnItemClickListener, OnClickListener {
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		if (info.isDPMLastMenber()) {
			...
			intent.putExtra(SelectMemberToDPMActivity.kTITLE, "分组成员");
	...
	private void addDPM(final String org_id, final String name) {
		if (name == null || TextUtils.isEmpty(name)) {
			QLToastUtils.showToast(AddDPMActivity.this, "分组名不能为空！");
			return;
		}
		showWaitDialog();
	...
	// 获取第一次获取分组列表,创建分组后，再次获取分组列表，进行对比，获取最近创建的分组ID，然后根据分组的ID，将成员拉进去
...
//				if (reply == null) {
//					QLToastUtils.showToast(AddDPMActivity.this, "分组创建成功,添加人失败");
//					finish();
//					return;
//				}
