...
public class AddImagesGridAdpter extends BaseAdapter {
    ...
    public AddImagesGridAdpter(ArrayList<String> paths, Activity activity, GridView gridView,int maxItem) {
        mPaths = paths;
        mActivity = activity;
        mGridView = gridView;
        mPaths.add("");
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.image = (SimpleDraweeView) convertView.findViewById(R.id.image_view_pick);
...
//            Picasso.with(holder.image.getContext()).load(imgFile).fit().centerCrop().into(holder.image);
            FrescoImageShowThumb.showThrumb(Uri.parse("file://"+mPaths.get(position)),holder.image);
            holder.del.setVisibility(View.VISIBLE);
            holder.del.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
