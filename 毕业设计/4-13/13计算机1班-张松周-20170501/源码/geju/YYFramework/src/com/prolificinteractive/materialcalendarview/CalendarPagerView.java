...
abstract class CalendarPagerView extends ViewGroup implements View.OnClickListener {
	...
	private HashMap<String, Date> flagDates = new HashMap<String, Date>();
	...
	public void flagDay(Date dt, boolean flag) {
		String key = QLDateUtils.getTimeWithFormat(dt, "yyyy-MM-dd");
		if (flag) {
			flagDates.put(key, dt);
		} else {
			flagDates.remove(key);
		}
	}

	public void frashDayView() {
		for (DayView dayView : dayViews) {
			String key = QLDateUtils.getTimeWithFormat(dayView.getDate().getDate(), "yyyy-MM-dd");
			if (flagDates.containsKey(key)) {
				Drawable flagDrawable = getResources().getDrawable(R.drawable.blue_point);
				flagDrawable.setBounds(0, 0, 5, 5);
				dayView.setCompoundDrawablePadding(2);
				dayView.setCompoundDrawables(null, null, null, flagDrawable);
			} else {
				dayView.setCompoundDrawables(null, null, null, null);
			}
		}
	...
	public void setFlagDates(Collection<CalendarDay> dates){
		for (DayView dayView : dayViews) {
			CalendarDay day = dayView.getDate();
			day.setOnDuty(dates != null && dates.contains(day));
			dayView.setDay(day);
		}
		postInvalidate();
	}
	...
	protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
		...
		// We expect to be somewhere inside a MaterialCalendarView, which should
		// measure EXACTLY
	...
	/**
	 * Simple layout params class for MonthView, since every child is the same
	 * size
	 */
