...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private boolean autoFlowTimer = false;
	private final int HANLER_AUTO = 9999;
	private final Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			QLLog.v("ViweFlow", "position="+position);
			QLLog.v("ViweFlow", "count="+count);
			if(count > 0)
				snapToScreen(mCurrentScreen+1%count);
			mHandler.sendEmptyMessageDelayed(HANLER_AUTO, timeSpan);
		}
	...
	public void startAutoFlowTimer(){
		autoFlowTimer = true;
		if(!mHandler.hasMessages(HANLER_AUTO)){
			mHandler.sendEmptyMessageDelayed(HANLER_AUTO, timeSpan);
		}
	}
	...
	public void stopAutoFlowTimer(){
		autoFlowTimer = false;
		mHandler.removeMessages(HANLER_AUTO);
	}
	
	private void pauseAutoFlowTimer(){
		if(autoFlowTimer && mHandler.hasMessages(HANLER_AUTO)){
			mHandler.removeMessages(HANLER_AUTO);
		}
	}
	
	private void resumeAutoFlowTimer(){
		startAutoFlowTimer();
	}
